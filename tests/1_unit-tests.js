...
suite('Unit Tests', function () {
  ...
  suite('Equality', function () {
    ...
    test('#equal, #notEqual', function () {
      ...
      assert.notEqual({ value: 1 }, { value: 1 }, '== compares object references');
      assert.equal(6 * 2, 12, "6 * '2' is coerced into 12");
      assert.notEqual(6 + '2', 12, "6 + '2' is coerced into '62'");
    });
    
    // #6
    test('#strictEqual, #notStrictEqual', function () {
      assert.notStrictEqual(6, '6', 'Strict comparison does not coerce types');
      assert.strictEqual(6, 3 * 2, '6 is strictly equal to 3 * 2');
      assert.strictEqual(6 * 2, 12, "6 * '2' coerces to 12, strictly equal");
      assert.notStrictEqual([1, 'a', {}], [1, 'a', {}], 'Arrays with same values but different references are not strictly equal');
    });
    // #7
